...
name = "Três Seis Nove"  # Nome do proprietário do bot
bot_name = "Isabel"  # Nome do bot
model_name = "gemini-1.5-flash"  # Modelo do Google Generative AI
...
def send(phone_number, answer):
    """Envia uma mensagem para o número do cliente."""
...
def webhook():
    if request.method == "GET":
        ...
        try:
            ...
            from_number = data["from"]  # Número do cliente que enviou a mensagem
            if data["type"] == "text":
                prompt = data["text"]["body"]
                convo.send_message(prompt)
                send(from_number, convo.last.text)
            else:
                ...
                if data["type"] == "audio":
                    ...
                    for _, page in enumerate(doc):
                        ...
                        convo.send_message(f"Resposta gerada pelo modelo com base no conteúdo enviado: {answer}")
                        send(from_number, convo.last.text)
                        remove(destination)
                else:
                    send(from_number, "Este formato não é suportado pelo bot ☹")
                ...
                convo.send_message(f"Mensagem processada pelo modelo: {answer}")
                send(from_number, convo.last.text)
